...
const UIProvider = (props: any) => {
  ...
  const value = useMemo(
    () => ({
      ...
      setSidebarView: (view: SIDEBAR_VIEWS) => dispatch({ type: 'SET_SIDEBAR_VIEW', view }),
      ...
      openModalBackInStock: () => dispatch({ type: 'OPEN_BACK_IN_STOCK_MODAL' }),
      closeModalBackInStock: () => dispatch({ type: 'CLOSE_BACK_IN_STOCK_MODAL' }),
    }),
    [state]
  );
...
export const ManagedUIContext = ({ children }: { children: React.ReactNode }) => <UIProvider>{children}</UIProvider>;
