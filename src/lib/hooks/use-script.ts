...
export interface UseScriptDataOptions {
  [key: string]: string;
}
...
function setDataOptions(dataOptions: UseScriptDataOptions, node: HTMLScriptElement) {
  Object.keys(dataOptions).forEach((key) => {
    node.setAttribute(`data-${key}`, dataOptions[key]);
  });
}

export function useScript(src: string | null, options?: UseScriptOptions, dataOptions?: UseScriptDataOptions): UseScriptStatus {
  ...
  useEffect(() => {
    ...
    if (!scriptNode) {
      ...
      // Add data options
      if (dataOptions) {
        setDataOptions(dataOptions, scriptNode);
      }
