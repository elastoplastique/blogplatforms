...
export const SignupForm = ({ onSubmit }: { onSubmit: any }) => {
  ...
  return (
    ...
    >
      ...
      <FormPrimitive.Field className="grid mb-[10px]" name="password" serverInvalid={serverErrors.password}>
        <div className="flex items-baseline justify-between">
          <FormPrimitive.Label className="text-[15px] font-medium leading-[35px] text-white">Password confirmation</FormPrimitive.Label>
          <FormPrimitive.Message className="text-[13px] text-white opacity-[0.8]" match="valueMissing">
            Please enter your password
          </FormPrimitive.Message>
          <FormPrimitive.Message className="text-[13px] text-white opacity-[0.8]" match="typeMismatch">
      ...
      <Text size="1" py="4">
        Have an account?{' '}
        <Link href={AUTH_LOGIN_PATHNAME} className="!underline">
          Login.
        </Link>
      </Text>
    </FormPrimitive.Root>
